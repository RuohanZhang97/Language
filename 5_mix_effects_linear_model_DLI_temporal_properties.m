...
%% linear regression model for interaction
NumWin = [NumWin_sensible_male NumWin_sensible_female NumWin_rational_male NumWin_rational_female];
Age = [Age_sensible_male Age_sensible_female Age_rational_male Age_rational_female];
...
Y = [State1_OccurrenceRate_sensible_male State1_OccurrenceRate_sensible_female State1_OccurrenceRate_rational_male State1_OccurrenceRate_rational_female];
Sex = [ones(1,length(State1_OccurrenceRate_sensible_male)) zeros(1,length(State1_OccurrenceRate_sensible_female)) ones(1,length(State1_OccurrenceRate_rational_male)) zeros(1,length(State1_OccurrenceRate_rational_female))];
ArticleClass = [zeros(1,length(State1_OccurrenceRate_sensible_male)) zeros(1,length(State1_OccurrenceRate_sensible_female)) ones(1,length(State1_OccurrenceRate_rational_male)) ones(1,length(State1_OccurrenceRate_rational_female))];
Interaction = Sex .* ArticleClass;
[~,~,r] = regress(Y',[ones(size(NumWin))' NumWin' Age']);
...
glme = fitglme(data,'y ~ 1 + sex + article_class + interaction + (1|subject)');
State1_OccurrenceRate_beta_pval = glme.Coefficients.pValue(2:end)';
State1_OccurrenceRate_beta_tval = glme.Coefficients.tStat(2:end)';
